...
Public Class Admin_Homevb
	...
	Private Sub LoadRevenueAndProfit(selectedDate As Date)
		Try
			...
			Dim grossCmd As New MySqlCommand("SELECT COALESCE(SUM(total_amount), 0) FROM orders WHERE DATE(order_date) = @d", conn)
			...
			Dim profitCmd As New MySqlCommand("
                ...
                WHERE DATE(o.order_date) = @d", conn)
	...
	Private Sub LoadOrderCount(selectedDate As Date)
		Try
			OpenConnection()
			Dim cmd As New MySqlCommand("SELECT COUNT(*) FROM orders WHERE DATE(order_date) = @d", conn)
	...
	Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
		Hide() : Admin_InvLogs.Show()
	End Sub
	Private Sub Button7_Click(sender As Object, e As EventArgs) Handles Button7.Click
		Me.Hide() : Admin_GrossProfit.Show()
	End Sub
End Class